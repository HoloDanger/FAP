...
public class AuthenticationService {
    ...
    public void encryptExistingPasswords() {
        try (Connection con = DriverManager.getConnection(url, dbUsername, dbPassword)) {
            ...
            try (PreparedStatement selectStmt = con.prepareStatement(selectQuery); ResultSet rs = selectStmt.executeQuery(); PreparedStatement updateStmt = con.prepareStatement(updateQuery)) {
    ...
    public int countUsernames() {
        int count = 0;
        try (Connection con = DriverManager.getConnection(url, dbUsername, dbPassword)) {
            String query = "SELECT COUNT(*) AS total FROM USER_INFO";
            try (PreparedStatement ps = con.prepareStatement(query); ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
            ...
            System.out.println("Failed to count usernames: " + e.getMessage());
        }
        return count;
    }
}
